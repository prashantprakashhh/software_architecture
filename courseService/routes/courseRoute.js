...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      // Create a new course object and then set createdBy
      const courseData = { ...req.body }; // Copy request body
      courseData.createdBy = req.user.userId; // Use userId from the JWT payload

      const course = new Course(courseData);
      await course.save();
      res.status(201).json(course);
    } catch (error) {
      // Log the full error for better debugging if it's a validation error
      if (error.name === 'ValidationError') {
        console.error("Course validation error:", error.errors);
        // Send back specific validation errors if helpful
